...
jQuery(document).ready(function() {
    ...
    //       var srclink = button.data('link'); // Extract info from data-* attributes
    //       // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
    //       // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
    //       var modal = $(this);
    //       modal.find('.modal-title').text(title);
    //       var attr = $(this).find('.modal-body video').attr('autoplay');
    //       if (typeof attr == typeof undefined || attr == false) {
    //         modal.find('.modal-body video').attr('autoplay','autoplay');    
    //       }
    //       modal.find('.modal-body video source').attr('src',srclink);
    //       $(".modal-video").load();
    // });

    // jQuery('#videoModal').on('hidden.bs.modal', function (event) {
    //     $(this).find('.modal-body video').removeAttr('autoplay');
    //     $(".modal-video").load();
    //     // return false;
    // });


    // jQuery('#readmoremodal').on('show.bs.modal', function (event) {
    //     var button = $(event.relatedTarget); // Button that triggered the modal
    //     var title = button.data('title') ;// Extract info from data-* attributes
    //     var imgsrc = button.data('imgsrc'); // Extract info from data-* attributes
    //     var desc = button.data('desc');
    //     var detailslink = button.data('link');
    //     var startdate = button.data('date');
    ...
    //     var modal = $(this);
    //     modal.find('.modal-title').text(title + "(" + duration + ")");
    //     modal.find('.modal-img').attr("ng-src",imgsrc);
    //     modal.find('.modal-desc').text(desc);
    //     modal.find('.modal-link').attr('href',detailslink);
    //     modal.find('.modal-date').text(startdate);
...
if ($('.kopa-countdown-2').length > 0) {
    Modernizr.load([{
        ...
        complete: function () {
          ...
          $('.kopa-countdown-2').countdown('2016/12/01 10:25:00', function(event) {
            var $this = $(this).html(event.strftime(''
                +'<li><div><span>days</span><h3>%D</h3><span>2016</span></div></li>'
                +'<li><div><span>hours</span><h3>%H</h3><span>2016</span></div></li>'
                +'<li><div><span>mins</span><h3>%M</h3><span>2016</span></div></li>'
                +'<li><div><span>secs</span><h3>%S</h3><span>2016</span></div></li>'));
          });
    ...
    Modernizr.load([{
        ...
        complete: function () {
            jQuery('.comment-form,.contact-form').validate({
                ...
                //             console.log("Form Returned success");
